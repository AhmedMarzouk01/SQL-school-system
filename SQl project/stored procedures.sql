...
as
begin
	-- Check if the instructor is authorized for the given course
	if exists (select 1 from instructor_course 
	where instructor_ID = @instructor_ID and course_ID = @course_ID)
	
	begin
	-- Check if the exam exists and is associated with the specified course
		...
		-- Assign the students to the exam with specified start and end times
...
begin
	...
	INSERT INTO Exam (Instructor_ID, Course_ID, Exam_Type, 
	Intake_ID, Branch_ID, Track_ID, Start_time, End_time, total_time, allowance_options)
    VALUES (@InstructorID, @CourseID, @ExamType, @Intake, @BranchID,
	@TrackID, @StartTime, @EndTime, @TotalTime, @AllowanceOptions)

	set @examID = SCOPE_IDENTITY()
	
	 -- Insert questions into the Exam_question table based on manual selection or random selection
	...
	-- Check if the total degree of the exam questions exceeds the maximum degree allowed for the course
...
BEGIN
	SET NOCOUNT ON
	 -- Check if the student is allowed to take the exam at the current time
...
(-- Type to hold question IDs and answers for finishing an exam
    QuestionID INT, answer nvarchar(max)
);

go
--- Procedure to finish an exam, evaluating student answers and updating their result
...
begin
	...
	begin
	 -- Determine the question type
		...
		where question_ID = @questionID
			SET @iscorrect = CASE 
    WHEN LTRIM(RTRIM(@student_answer)) = LTRIM(RTRIM(@correctanswer)) OR  ---Exact Match
	---This checks if the trimmed correct answer appears anywhere within the trimmed student answer.
         LTRIM(RTRIM(@student_answer)) LIKE '%'+LTRIM(RTRIM(@correctanswer))+'%' OR
		 ----This checks if the trimmed student answer starts with the trimmed correct answer.
         LTRIM(RTRIM(@student_answer)) LIKE LTRIM(RTRIM(@correctanswer))+'%' OR
		 -------This checks if the trimmed student answer ends with the trimmed correct answer.
         LTRIM(RTRIM(@student_answer)) LIKE '%'+LTRIM(RTRIM(@correctanswer)) OR
		 ------This checks if the trimmed correct answer is found within the trimmed student answer.
         CHARINDEX(LTRIM(RTRIM(@correctanswer)), LTRIM(RTRIM(@student_answer))) > 0 OR
		 -----This checks if the trimmed student answer is found within the trimmed correct answer.
         CHARINDEX(LTRIM(RTRIM(@student_answer)), LTRIM(RTRIM(@correctanswer))) > 0 OR
		 -----This checks if the trimmed correct answer appears anywhere within the trimmed student answer using pattern matching.
		...
		-- Insert the evaluated answer into Student_answer table
...
set @result = case 
	...
	---insert the result of the student
update Student_exam
set result = @result
...
----5- get exam result for student 
go

create proc getexam_result_for_student
	@studentID int
as 
begin
	select*
